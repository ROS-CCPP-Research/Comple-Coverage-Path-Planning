...
class BaseOptimizer(object):
    ...
    def solve(cls, navigable_area, resolution):
        ...
        n_agents = np.clip(n_agents, a_min=2, a_max=cls.CARDINALITY_GAIN).astype(dtype=np.uint8)
...
class OptimizerFromOccupancyGrid(BaseOptimizer):
    ...
    def run(self):
        ...
        n_agents = OptimizerFromOccupancyGrid.solve(area, occupancy_grid.info.resolution)
        self.pub.publish(n_agents)
