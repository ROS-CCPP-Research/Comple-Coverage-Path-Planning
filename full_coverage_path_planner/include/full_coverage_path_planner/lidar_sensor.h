...
private:
    ...
    void get_map();
    void get_params();
    void lidarCallback(const sensor_msgs::LaserScan::ConstPtr& msg, const std::string& robot_name);
    /*! finds the pose of the laser in the map frame */
    void get_laser_pose(double * x, double * y, double * theta, std::string robot_name);
    ...
    int robotCount;
    std::vector<ros::Subscriber> lidarSubscribers; // Vector to hold subscribers
