...
int main(int argc, char** argv)
{
    ...
    sub_Pose.pose.position.x = start_position.x;
    sub_Pose.pose.position.y = start_position.y;
    ...
    sub_Scaled.x = static_cast<unsigned int>(clamp((sub_Pose.pose.position.x - occ_grid.info.origin.position.x) / dmax(floor(tool_radius / occ_grid.info.resolution), 1) * occ_grid.info.resolution, 0.0,
                             floor(occ_grid.info.width / dmax(floor(tool_radius / occ_grid.info.resolution), 1) * occ_grid.info.resolution)));
    sub_Scaled.y = static_cast<unsigned int>(clamp((sub_Pose.pose.position.y - occ_grid.info.origin.position.y) / dmax(floor(tool_radius / occ_grid.info.resolution), 1) * occ_grid.info.resolution, 0.0,
