...
{
    nh_ptr->param<std::string>("robot_namespace", robot_namespace, "robot_");
    nh_ptr->param<double>("laser_frequency", l_frequency, 10.0);

    nh_ptr->param<int>("robot_count", robotCount, 4);

    nh_ptr->param<std::string>("map_service", map_service, "map");
...
void LaserScannerSimulator::start() {
    // Assuming robotCount is already set
    lidarSubscribers.resize(robotCount); // Resize the vector to hold all subscribers

    for (size_t i = 0; i < robotCount; ++i) {
        std::string topic = robot_namespace + std::to_string(i) + "/scan";
        // Use a lambda function to capture the robot's name and pass it to the callback
        lidarSubscribers[i] = nh_ptr->subscribe<sensor_msgs::LaserScan>(topic, 10, [this, i](const sensor_msgs::LaserScan::ConstPtr& msg) {
            this->lidarCallback(msg, robot_namespace + std::to_string(i));
        });
...
void LaserScannerSimulator::lidarCallback(const sensor_msgs::LaserScan::ConstPtr& msg, const std::string& robot_name) {
    ROS_INFO_STREAM("Received scan from " << robot_name);
    
    int centerIndex = msg->ranges.size() / 2;
    float distanceToObstacle = msg->ranges[centerIndex];

    // Define a threshold distance to consider an obstacle as blocking the path
    float thresholdDistance = 1.0; // meters

    if (distanceToObstacle < thresholdDistance) {
        ROS_INFO_STREAM("Obstacle detected in front of " << robot_name << " at distance: " << distanceToObstacle << " meters");
        double l_x, l_y, l_theta;
        get_laser_pose(&l_x,&l_y,&l_theta,robot_name);
        ROS_INFO_STREAM("x : "<<l_x<<" "<<"y : "<<l_y<<" "<<"theta : ");
    } else {
        ROS_INFO_STREAM("No obstacle in front of " << robot_name << " within " << thresholdDistance << " meters");
    }
}

void LaserScannerSimulator::get_laser_pose(double * x, double * y, double * theta, std::string robot_name)
{
    ros::Time now = ros::Time::now();
    tf::StampedTransform transf;
    try
    {
        tl.waitForTransform("/map",robot_name +"/"+ l_frame,now,ros::Duration(1.0));
        tl.lookupTransform("/map",robot_name +"/"+ l_frame,now,transf);
    }
